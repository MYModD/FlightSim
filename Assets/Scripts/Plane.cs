...
public class Plane : MonoBehaviour
{
    ...
    [Header("Lift")]  // 揚力に関する設定
    [SerializeField]
    float liftPower;
    [SerializeField]
    AnimationCurve liftAOACurve;  // 迎え角に対する揚力係数
    [SerializeField]
    float inducedDrag;
    [SerializeField]
    ...
    [Header("Misc")]  // その他の設定
    [SerializeField]
    List<Collider> landingGear;  // 着陸装置
    [SerializeField]
    PhysicMaterial landingGearBrakesMaterial;
    [SerializeField]
    List<GameObject> graphics;
    [SerializeField]
    GameObject damageEffect;  // ダメージエフェクト
    [SerializeField]
    GameObject deathEffect;  // 死亡エフェクト
    [SerializeField]
    bool flapsDeployed;
    [SerializeField]
    float initialSpeed;

    [Header("Weapons")]  // 武器に関する設定
    [SerializeField]
    List<Transform> hardpoints;  // ハードポイント
    [SerializeField]
    float missileReloadTime;  // ミサイルのリロード時間
    [SerializeField]
    float missileDebounceTime;  // ミサイルのデバウンス時間
    [SerializeField]
    GameObject missilePrefab;  // ミサイルのPrefab
    [SerializeField]
    Target target;  // ターゲット
    [SerializeField]
    float lockRange;  // ロックレンジ
    [SerializeField]
    float lockSpeed;  // ロックスピード
    [SerializeField]
    float lockAngle;  // ロックアングル
    [SerializeField]
    [Tooltip("Firing rate in Rounds Per Minute")]
    float cannonFireRate;  // キャノンの発射レート
    [SerializeField]
    float cannonDebounceTime;  // キャノンのデバウンス時間
    [SerializeField]
    float cannonSpread;  // キャノンのばらつき
    [SerializeField]
    Transform cannonSpawnPoint;  // キャノンのスポーンポイント
    [SerializeField]
    GameObject bulletPrefab;  // 弾丸のPrefab

    new PlaneAnimation animation;

    float throttleInput;  // スロットル入力
    Vector3 controlInput;  // コントロール入力
    ...
    void CalculateState(float dt)
    {
        ...
        LocalVelocity = invRotation * Velocity;  // ワールド座標からローカル座標に変換
        LocalAngularVelocity = invRotation * Rigidbody.angularVelocity;  // ローカル座標に変換
    ...
    void UpdateDrag()
    {
        ...
        var coefficient = Utilities.Scale6(
            ...
            dragForward.Evaluate(Mathf.Abs(lv.z)) + airbrakeDrag + flapsDrag,   // 前方係数に追加抗力を含む
            dragBack.Evaluate(Mathf.Abs(lv.z))
        );

        var drag = coefficient.magnitude * lv2 * -lv.normalized;    // 抗力は速度と反対方向
    ...
    {
        var liftVelocity = Vector3.ProjectOnPlane(LocalVelocity, rightAxis);    // 速度をYZ平面に投影
        var v2 = liftVelocity.sqrMagnitude;                                     // 速度の二乗
    ...
    void UpdateAngularDrag()
    {
        ...
        var drag = av.sqrMagnitude * -av.normalized;    // 二乗し、角速度と反対方向
        Rigidbody.AddRelativeTorque(Vector3.Scale(drag, angularDrag), ForceMode.Acceleration);  // 質量を無視
    }
    ...
    Vector3 CalculateGForceLimit(Vector3 input)
    {
        return Utilities.Scale6(input,
            gLimit, gLimitPitch,    // ピッチダウン、ピッチアップ
            gLimit, gLimit,         // ヨー
            gLimit, gLimit          // ロール
        ) * 9.81f;
    }
    ...
    float CalculateGLimiter(Vector3 controlInput, Vector3 maxAngularVelocity)
    {
        ...
        // 入力の大きさが1未満の場合、入力をスケールアップして大きさを1にする
    ...
    void UpdateSteering(float dt)
    {
        ...
        Rigidbody.AddRelativeTorque(correction * Mathf.Deg2Rad, ForceMode.VelocityChange);    // 質量を無視
    ...
    void UpdateMissileLock(float dt)
    {
        ...
        if (Target != null && !Target.Plane.Dead)
        {
            ...
            var errorDir = Quaternion.Inverse(Rigidbody.rotation) * error.normalized; // ローカル座標に変換
